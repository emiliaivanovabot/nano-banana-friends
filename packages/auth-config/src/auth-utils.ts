...
export async function authenticateUser(username: string, password: string): Promise<AuthResult> {
  try {
    ...
    if (dbError || !user) {
      ...
      const loginUsers = process.env.VITE_LOGIN_USERS || import.meta?.env?.VITE_LOGIN_USERS
      const localUsers = JSON.parse(loginUsers || '[]')
...
export const SessionManager = {
   ...
   */
  setSession(user: User, requiresOnboarding = false): boolean {
    try {
