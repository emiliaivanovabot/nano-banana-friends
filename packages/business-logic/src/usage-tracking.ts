...
export const updateDailyUsage = async ({
  ...
  outputTokens = 0,
  costUsd = 0
}: UsageUpdateParams): Promise<UsageResult> => {
  try {
    ...
    if (existingRecord) {
      ...
      // Create new record
      const newRecord: Omit<DailyUsageRecord, 'id'> = {
        user_id: userId,
        usage_date: today,
        cost_usd: costUsd,
 ...
 */
export const incrementErrorCount = async (username: string): Promise<UsageResult> => {
  try {
    const supabase = getSupabaseClient()
 ...
 */
export const getDailyUsageHistory = async (userId: string, days = 30): Promise<{
  success: boolean
  data: DailyUsageRecord[]
  error?: string
}> => {
 ...
 */
export const getTopAspectRatios = async (
  userId: string, 
  days = 30, 
  limit = 2
): Promise<{
 ...
 */
export const getUnifiedGenerationStats = async (
  userId: string, 
  days = 90
): Promise<{
  success: boolean
  ...
  try {
    ...
    const processedData: Record<string, DailyUsageRecord> = {}
